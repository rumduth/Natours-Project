...
const tourSchema = new mongoose.Schema(
  ...
  },
  {
    toJSON: {
      virtuals: true,
      transform: (doc, ret) => {
        delete ret.id; // Remove `id` from the JSON output
      },
    }, // Disables `id` virtual
    toObject: {
      virtuals: true,
...
//Document middleware, it runs before the save commamand and the .create() command
...
tourSchema.pre("aggregate", function (next) {
  let k = Object.keys(this.pipeline().at(0))?.at(0);
  if (k === "$geoNear") {
    return next();
  }
